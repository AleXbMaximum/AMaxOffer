...
(function () {
    ...
    const StatsManager = (() => {
        ...
        return {
            ...
            calculateAll() {
                this.calculateMembersStats();
                this.calculateOffersStats();
                // Benefits stats are calculated on-demand with the actual data
            },
            ...
            invalidate(type) {
                ...
                } else if (statsCache.hasOwnProperty(type)) {
                    statsCache[type] = null;
                }
            },

            calculateMembersStats() {
                ...
                const financialData = glb_account
                ...
                statsCache.members = {
                    ...
                    totalRemaining: financialData.totalRemaining
                };

                return statsCache.members;
            },

            calculateOffersStats() {
                const enrollmentStats = glb_offer.reduce((stats, offer) => {
                ...
                statsCache.offers = {
                    ...
                    favoriteOffers,
                    expiringSoon,
                    distinctFullyEnrolled: enrollmentStats.distinctFullyEnrolled,
                    distinctNotFullyEnrolled: enrollmentStats.distinctNotFullyEnrolled,
                    totalEligible: enrollmentStats.totalEligible,
                    totalEnrolled: enrollmentStats.totalEnrolled
                };

                return statsCache.offers;
            },

            getMembersStats() {
                if (!statsCache.members) {
                    this.calculateMembersStats();
                }
                return statsCache.members;
            },
            ...
            getBenefitsStats(benefits) {
                return {
                    ...
                    achieved: benefits.filter(b => b.status === 'ACHIEVED').length,
                    inProgress: benefits.filter(b => b.status === 'IN_PROGRESS').length,
                    notStarted: benefits.filter(b => b.status === 'NOT_STARTED' || (!b.status && parseFloat(b.tracker?.spentAmount) <= 0)).length
                };
            }
        };
    })();
    ...
    async function api_fetchAccounts(readonly = false) {
        ...
        if (Array.isArray(glb_account) && glb_account.length > 0) {
            ...
            StatsManager.invalidate('members');
            return true;
        }
        ...
        await Promise.all(basicAccounts.map(async (acc) => {
            ...
            if (Array.isArray(trackers)) {
                trackers.forEach(tracker => {
                    tracker.accountToken = acc.account_token;
    ...
    async function api_batchEnrollOffers(offerSourceId, accountToken) {
        ...
        const batchSize = Math.min(runInBatchesLimit, 25);
        const results = [];
        ...
        for (let i = 0; i < tasks.length; i += batchSize) {
            ...
            const batchPromises = batch.map(async task => {
                ...
                try {
                    ...
                    if (result.result) {
                        const offer = glb_offer.find(o => o.source_id === task.sourceId);
                        if (offer) {
        ...
        sortedOffers.forEach((offer, index) => {
            ...
            if (type === 'redeemed' && offer.redeemedCards) {
                ...
                offer.redeemedCards.forEach((token, i) => {
                    ...
                    const account = glb_account.find(acc => acc.account_token === token);
    ...
    function offers_renderControlBar() {
        ...
        const cardFilter = ui_createReactiveFilter(cardSearchContainer, {
            ...
            initialValue: glb_filters.offerCardToken || '',
            onFilterChange: (value) => {
                glb_filters.offerCardToken = value;
    ...
    function offers_toggleFavorite(offer) {
        ...
        offer.__processed_favorite = offer.favorite ?
            '<svg width="18" height="18" viewBox="0 0 24 24" fill="#ff9500"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg>' :
            '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#777" stroke-width="2"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg>';
        ...
        // Update UI to reflect changes if the stats bar is visible
    ...
    const TableDataProcessor = {
        processTableData(tableId, rawData, options = {}) {
            ...
            if (sortState && sortState.key) {
                ...
                const arrayColumns = ['eligibleCards', 'enrolledCards'];

                processedItems.sort((a, b) => {
                    if (arrayColumns.includes(sortState.key)) {
                        // Handle arrays by comparing their lengths
                        const lenA = Array.isArray(a[sortState.key]) ? a[sortState.key].length : 0;
                        const lenB = Array.isArray(b[sortState.key]) ? b[sortState.key].length : 0;
                        return sortState.direction * (lenA - lenB);
                    } else if (numericColumns.includes(sortState.key)) {
                        ...
                        // Special case for favorite column
    ...
    function benefits_createFilters() {
        ...
        function createCardFilter() {
            ...
            const uniqueAccounts = [...new Set(glb_benefit.map(benefit => benefit.accountToken))]
                .filter(Boolean)
                .map(token => {
                    const account = glb_account.find(acc => acc.account_token === token);
                    return account ? {
    ...
    function benefits_processAndGroup(benefits) {
        ...
        const groupedBenefits = benefits.reduce((grouped, trackerObj) => {
            ...
            if (!trackerObj.cardEnding && trackerObj.accountToken) {
                const account = glb_account.find(acc => acc.account_token === trackerObj.accountToken);
                if (account) {
    ...
    function benefits_createProgressCard(trackerObj, groupObj) {
        const cardAccount = glb_account.find(acc => acc.account_token === trackerObj.accountToken);
