...
// @description  AMaxOffer Offers and Account Management Tool for American Express Site – Vue Display Version
...
(function () {
    ...
    async function renderCurrentView() {
        // Save scroll state and apply sorts as before…
        ...
        if (currentView === 'summary') {
            // Create a container for the Vue summary view and append it
            const vueContainer = document.createElement('div');
            vueContainer.id = 'vue-summary';
            content.appendChild(vueContainer);

            // Mount the Vue instance using a render function (avoids the template compiler)
            new Vue({
                el: '#vue-summary',
                data: {
                    ...
                    // Define style objects to mimic your original styling
                    styles: {
                        summary: {
                            fontSize: '16px',
                            lineHeight: '1.5',
                ...
                computed: {
                    summary() {
                        const numAccounts = this.accountData.length;
                        const updateTime = this.lastUpdate || "Never";
                ...
                methods: {
                    async refreshData() {
                        ...
                        if (fetchStatus) {
                            ...
                            await renderCurrentView();
                            setLocalStorage(this.accountData[0].account_token, ["accountData", "offerData", "lastUpdate", "benefitTrackers"]);
                        }
                    },
                    async enrollAll() {
                ...
                render(h) {
                    return h('div', { style: this.styles.summary }, [
                        h('p', [
                            h('strong', 'Number of Accounts: '),
                            String(this.summary.numAccounts),
                            "    ",
                            h('strong', 'Last Update: '),
                            String(this.summary.updateTime)
                        ]),
                        h('p', [
                            h('strong', 'Distinct Offers Fully Enrolled: '),
                            String(this.summary.distinctFullyEnrolled),
                            "    ",
                            h('strong', 'Total Offers Enrolled: '),
                            String(this.summary.totalEnrolled)
                        ]),
                        h('p', [
                            h('strong', 'Distinct Offers Not Fully Enrolled: '),
                            String(this.summary.distinctNotFullyEnrolled),
                            "    ",
                            h('strong', 'Total Offers Eligible: '),
                            String(this.summary.totalEligible)
                        ]),
                        h('div', { style: this.styles.btnContainer }, [
                            h('button', {
                                style: this.styles.button,
                                on: { click: this.enrollAll }
                            }, 'Enroll All'),
                            h('button', {
                                style: this.styles.button,
            ...
            const benefitsView = await renderBenefitsView();
            content.appendChild(benefitsView);
        }

        // Restore previous scroll position if applicable
